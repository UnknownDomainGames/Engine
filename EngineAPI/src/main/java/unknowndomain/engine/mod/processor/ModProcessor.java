package unknowndomain.engine.mod.processor;

import com.google.common.base.Strings;
import unknowndomain.engine.mod.JsonModMetadataSerializer;
...
public class ModProcessor extends AbstractProcessor {
    ...
    private void saveMetadata(TypeElement element) {
        ...
        try (Writer writer = fileObject.openWriter()) {
            ...
            writer.append(JsonModMetadataSerializer.toJson(ModMetadata.Builder.create().id(modIdentifier.getId()).version(modIdentifier.getVersion()).mainClass(element.getQualifiedName().toString()).build()).toString());
        } catch (IOException e) {
            processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, e.getMessage());
            e.printStackTrace();
        }
