...
public class FrameBufferMultiSampled extends FrameBuffer {
    ...
    public void resize(int width, int height) {
        ...
        this.width = width;
        this.height = height;
        GL11.glBindTexture(GL32.GL_TEXTURE_2D_MULTISAMPLE, texPointer);
        ...
        bind();
        GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0, GL32.GL_TEXTURE_2D_MULTISAMPLE, texPointer, 0);
        if (dstexPointer == -1) {
            dstexPointer = GL11.glGenTextures();
        }

        GL11.glBindTexture(GL32.GL_TEXTURE_2D_MULTISAMPLE, dstexPointer);

        GL32.glTexImage2DMultisample(GL32.GL_TEXTURE_2D_MULTISAMPLE, 4, GL30.GL_DEPTH24_STENCIL8, width, height, true);
        GL11.glBindTexture(GL32.GL_TEXTURE_2D_MULTISAMPLE, 0);

        GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_DEPTH_STENCIL_ATTACHMENT, GL32.GL_TEXTURE_2D_MULTISAMPLE, dstexPointer, 0);
        unbind();
    }
}
