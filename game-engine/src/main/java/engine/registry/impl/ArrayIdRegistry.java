...
@NotThreadSafe
public abstract class ArrayIdRegistry<T extends Registrable<T>> extends BaseRegistry<T> {

    protected T[] idToObject;

    public ArrayIdRegistry(Class<T> entryType) {
        this(entryType, entryType.getSimpleName().toLowerCase());
    }

    public ArrayIdRegistry(Class<T> entryType, String name) {
