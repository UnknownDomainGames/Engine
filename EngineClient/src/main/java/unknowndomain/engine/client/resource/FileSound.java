...
public class FileSound extends Sound {
    ...
    public boolean loadSound() {
        ...
        try (
            ...
            Platform.getLogger().warn("Cannot load sound! (file not found) file: {}", file.getAbsolutePath());
            return false;
        } catch (IOException e) {
            Platform.getLogger().warn("Cannot load sound! (IO exception) file: {}", file.getAbsolutePath());
            return false;
        } finally {
            if (buf != null) {
                buf.clear();
