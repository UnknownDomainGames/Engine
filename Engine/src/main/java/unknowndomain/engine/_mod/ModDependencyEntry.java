...
import unknowndomain.engine._mod.exception.DependencyParseException;
...
public class ModDependencyEntry {
    ...
    public static ModDependencyEntry parse(String spec) {
        ...
        if (args.length != 3) {
            throw new DependencyParseException("Failed to parse dependency entry. Source: " + spec);
        }
        ...
        try {
            ...
            throw new DependencyParseException("Failed to parse dependency entry, invalid version range. Range: " + args[2], e);
        } catch (IllegalArgumentException e) {
            throw new DependencyParseException("Failed to parse dependency entry, illegal load order. Load order: " + args[0], e);
        }
    }
